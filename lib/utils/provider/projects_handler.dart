...
class ProjectsHandler extends ChangeNotifier {
  ...
  Future<bool> connect() async {
    ...
    if (_user != null) {
      _googleDrive.initDriveApi(user: _user!);
    }
  ...
  Future<void> uploadAttachment(
    ...
    _googleDrive.upload(folderName: folderName, data: data, fileName: fileName);
  }
  // ------------------------------ Profile Image -----------------------------

  Future<bool> get doesProfileImageExists async {
    String path =
        'users/${_firebaseAuth.currentUser?.email}/profile_image/${_firebaseAuth.currentUser?.email}.jpg';
  ...
  Future<Uint8List?> get _getProfileImage {
  ...
  Future<String> setData(Map<String, dynamic> newData) async {
    try {
